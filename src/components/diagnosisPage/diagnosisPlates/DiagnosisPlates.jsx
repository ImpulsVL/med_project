    ...
    return (
        <div className='diagnosis_plates_wrapper'>
            {diagnoses.map((diagnosis, index) => (
                <Link className='diagnosis_plate_second_page' to={`/diagnos/${diagnosis.id}/${sectionCode}`} key={index} state={{ diagnosisId: diagnosis.id, code: diagnosis.code, name: diagnosis.name }}>
